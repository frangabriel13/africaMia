...
function ProductForm({ getProducts }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Redimensiona las imágenes antes de enviarlas al servidor
    const resizedImages = await Promise.all(formData.images.map(async (image) => {
      return await resizeImage(image);
    }));

    // Actualiza el formData con las imágenes redimensionadas
    const updatedFormData = { ...formData, images: resizedImages };

    // Envía el formulario
    await dispatch(addProduct(updatedFormData));
  ...
  // Agrega más funciones de manejo de eventos según sea necesario
